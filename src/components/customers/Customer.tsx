// file htmlFor customer

// importing the required modules
import React, { useState, useEffect, useCallback } from "react";
import { debounce } from "lodash";
import { Table, TextInput, Card, Spinner, Select } from "flowbite-react";
    ...
    <Card className="max-w-7xl mx-auto mt-8 p-6 shadow-lg border border-gray-200 dark:border-gray-700 rounded-lg">
      ...
      <div className="relative overflow-x-auto rounded-lg">
        {isLoading ? (
          ...
          <Table hoverable>
            <Table.Head>
              <Table.HeadCell className="text-center">Sl. No.</Table.HeadCell>
            ...
            <Table.Body className="divide-y">
              {tableData.map((item, index) => (
                ...
                >
                  <Table.Cell className="text-center font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    {index + 1}
                  </Table.Cell>
                  <Table.Cell>{item.name}</Table.Cell>
                  <Table.Cell className="text-center">{item.dob}</Table.Cell>
      ...
      {tableData && tableData.length > 0 && (
        <div className="flex flex-row space-x-5 mt-4 justify-center">
          <button
